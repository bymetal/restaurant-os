import type { FastifyInstance, FastifyRequest } from "fastify";
import type { Pool } from "pg";
import type { TelegramConfig } from "@restaurant-os/integrations";
import { ApiError } from "../errors.js";
import { disconnect, getConnection, startLinking } from "../repositories/telegram.js";

export function registerTelegramRoutes(
  app: FastifyInstance,
  pool: Pool,
  telegramConfig: TelegramConfig,
  appUrl: string,
  webhookSecret: string
): void {
  app.get(
    "/v1/integrations/telegram",
    { preHandler: [app.authenticate, app.requireScope("business"), app.requirePermission("business:integration:read")] },
    async (request) => {
      const context = requireBusiness(request);
      return { connection: await getConnection(pool, context.businessId) };
    }
  );

  app.post(
    "/v1/integrations/telegram/connect",
    { preHandler: [app.authenticate, app.requireScope("business"), app.requirePermission("business:integration:write")] },
    async (request) => {
      const context = requireBusiness(request);
      const connection = await startLinking(pool, context.businessId, telegramConfig, appUrl, webhookSecret, {
        userId: context.userId,
        role: context.role,
        ipAddress: request.ip,
        userAgent: request.headers["user-agent"]
      });
      return { connection };
    }
  );

  app.post(
    "/v1/integrations/telegram/disconnect",
    { preHandler: [app.authenticate, app.requireScope("business"), app.requirePermission("business:integration:write")] },
    async (request) => {
      const context = requireBusiness(request);
      await disconnect(pool, context.businessId, {
        userId: context.userId,
        role: context.role,
        ipAddress: request.ip,
        userAgent: request.headers["user-agent"]
      });
      return { connected: false };
    }
  );
}

function requireBusiness(request: FastifyRequest) {
  if (!request.auth || request.auth.scope !== "business" || !request.auth.businessId) {
    throw new ApiError(403, "FORBIDDEN", "Business context is required.");
  }
  return request.auth as FastifyRequest["auth"] & { businessId: string };
}
